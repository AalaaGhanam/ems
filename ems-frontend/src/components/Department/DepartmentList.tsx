...
const DepartmentList = ({ departments }: ListProps) => {
    ...
    const isEditing = (record: DepartmentPropsTypes) =>
    ...
    const save = async (key: React.Key) => {
        try {
            ...
            const index = newData.findIndex(
                (item) => Number(key) === Number(item.key)
            )
            const department = newData.find(
                (item: any) => Number(item.key) === index
            )
            try {
                await dispatch(
                    editDepartment({
    ...
    const mergedColumns: TableProps<DepartmentPropsTypes>['columns'] =
        columns.map((col) => {
            if (!col.editable) {
                return col
            }
